...
for (const locale of ["he", "en"] as const) {
  test(`client logos loop in one colorful row and support pausing (${locale})`, async ({ page }) => {
    ...
    await expect(list.locator('li[aria-hidden="true"]')).toHaveCount(1);
    ...
    await expect(list.locator("img").first()).toHaveCSS("filter", "none");
    ...
    await expect(region.locator('ul[aria-hidden="true"]')).toBeHidden();
