...
export const post = defineType({
  ...
  fields: [
    ...
    defineField({ name: "read", title: "זמן קריאה", type: "string", group: "meta", description: "לדוגמה: 4 דק׳ קריאה", validation: (rule) => rule.required() }),
    defineField({
      name: "category",
      title: "קטגוריה",
      type: "string",
    ...
    defineField({ name: "excerpt", title: "תקציר", type: "text", rows: 2, group: "content", validation: (rule) => rule.required() }),
    defineField({ name: "body", title: "פסקאות", type: "array", of: [{ type: "text", rows: 3 }], group: "content", validation: (rule) => rule.required().min(1) }),
  ],
  preview: {
