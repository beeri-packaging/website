...
} from "../catalog-example/CatalogProductConcept";
import type { CatalogExample } from "../catalog-example/content";
import styles from "../catalog-example/catalog-example.module.css";
import {
  catalogPreviewCategories,
  catalogPreviewCopy as copy,
} from "./content";
import { catalogPreviewLocalExamples } from "./examples";

export const metadata: Metadata = {
  title: "תצוגת קטלוג מוצרים · בארי אריזות",
  description: "גרסת תצוגה נפרדת לקטלוג המבוסס על סוגי מוצרים וגלריות ביצוע.",
  robots: { index: false, follow: false },
};

export default async function CatalogPreviewPage() {
  const catalog = toCatalogContent(await getCatalog("he"), "he");
  const examplesByKey = new Map<string, CatalogExample>([
    ...catalog.categories
    ...
    ...catalogPreviewLocalExamples.map((item) => [item.key, item] as const),
  ]);

  const categories: CategoryWithProducts[] = catalogPreviewCategories.map(
    (category) => ({
      key: category.key,
    ...
    <main id="main" className="min-h-screen bg-bone pb-24 text-ink sm:pb-32">
      <header className="border-b border-ink">
        <Container className="flex items-center justify-between gap-5 py-4">
          ...
          <p className="text-end font-sans text-[11px] font-bold tracking-[0.04em] text-clay sm:text-[12px]">
            {copy.headerLabel}
          </p>
        </Container>
      </header>
      ...
      <CatalogProductConcept categories={categories} imageFit="contain" emptyExamplesLabel={copy.emptyExamplesLabel} />
    </main>
  );
}
